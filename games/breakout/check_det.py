"""Determinism check for breakout: two pages, same seed, same action sequence.

Compares score, done, t, info() and the encoded frame at every step, for several seeds.
Run:  source .venv/bin/activate && python games/breakout/check_det.py
"""
import asyncio
import json
import random
...
async def main() -> int:
    ...
    async with VecGame(GAME, n=2) as env:
        for seed in SEEDS:
            ...
            if obs[0]["done"] or obs[1]["done"]:
                bad.append(f"seed {seed}: done right after start")
            if json.dumps(obs[0].get("info"), sort_keys=True) != json.dumps(obs[1].get("info"), sort_keys=True):
                bad.append(f"seed {seed} start: info differs")
            ...
            print(f"seed {seed}: {steps} steps, score {obs[0]['score']}, done={obs[0]['done']}, "
                  f"level={i['level']}, bricks_left={i['bricks_left']}, t={obs[0]['t']:.0f}ms")
    if bad:
